...
bar_matrix <- read.table("/Users/bo/Documents/data_yuji_lab/data_MEG/eyeData_script_result/position_mri/meg_fix_sac_visual_angle_back_1s_with_correction.txt", stringsAsFactors=FALSE)
...
figure <- ggplot(data=table, aes(x=coor, y=mean, fill=ego)) +  
  ...
  scale_y_continuous(labels=scaleFUN, limits=c(-1.5, 4)) +
  #ggtitle(paste("","\n\n", sep = "")) +
  theme(axis.title.y = element_text(size=font_size))+
  theme(axis.title.x = element_text(size=font_size))+
  theme(legend.text=element_text(size=font_size), legend.direction = "horizontal", legend.position=c(0.5,.9))+
...
ggsave(paste("/Users/bo/Desktop/meg_va_fixation.pdf"), #meg_beh.pdf mri_beh.pdf
...
figure <- ggplot(data=table, aes(x=coor, y=mean, fill=ego)) +  
  ...
  scale_fill_manual("legend", values = c("Back" = "dodgerblue4", "Left" = "gold4", "Right" = "goldenrod1")) +
  theme_classic() +  
  labs(x="",y="\n") + #Saccade Va.of saccade
  scale_y_continuous(labels=scaleFUN, limits=c(-1.5, 3)) +
  theme(axis.title.y = element_text(size=font_size))+
  theme(axis.title.x = element_text(size=font_size))+
  theme(legend.text=element_text(size=font_size-4), legend.direction = "horizontal", legend.position=c(0.5,.9))+
  theme(plot.title = element_text(hjust = 0.5)) + 
  theme(axis.text.x = element_text(size=font_size, angle = 20, vjust = 0.5)) + 
  theme(axis.text.y = element_text(size=font_size, angle = 0, vjust = 0.5)) + 
  theme(legend.title=element_blank()) 
  #+guides(fill=FALSE)
ggsave(paste("/Users/bo/Desktop/meg_va_saccade.pdf"), #meg_beh.pdf mri_beh.pdf
       plot = figure, # or give ggplot object name as in myPlot,
       width = 6.5, height = 5,
       units = "in", # other options c("in", "cm", "mm"),
       dpi = 300)
